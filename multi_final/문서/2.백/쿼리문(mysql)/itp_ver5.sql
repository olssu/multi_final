...
insert into itp_notice values(2,"사이트 점검안내","2022년 3월 18일~19일 점검 예쩡입니다.",now());
...
create table itp_group(
    ...
    foreign key(g_name) references itp_member(m_name) on DELETE CASCADE
    );
...
c_date date default (current_date),
 foreign key(c_name) references itp_member(m_name) on DELETE CASCADE
 );
...
foreign key(rg_rno) references itp_group(g_no) on DELETE CASCADE,
 foreign key(rg_name) references itp_member(m_name) on DELETE CASCADE);
...
foreign key(r_rno) references itp_com(c_no) on DELETE CASCADE,
 foreign key(r_name) references itp_member(m_name) on DELETE CASCADE
);
...
a_gno int not null,
g_name varchar(200) not null,
a_auth varchar(200) default '확인중' ,
 foreign key(a_name) references itp_member(m_name) on DELETE CASCADE,
 foreign key(a_gno) references itp_group(g_no) on DELETE CASCADE
);

insert into itp_apply values(1,'Maple','Maple@naver.com','신청해보고 싶어요! 프론트엔드개발자입니다',13,'Carrie',default);
insert into itp_apply values(2,'Frank','Frank@naver.com','정처기필기시험 준비중입니다 참여하고싶어요',2,'Mary',default);
insert into itp_apply values(3,'Mary','Mary@naver.com','참여하고싶어요',15,'Ryan',default);
insert into itp_apply values(4,'Margaret','Margaret@naver.com','참여',14,'Mary',default);
...
create table itp_tech( 
    ...
    t_date date default (current_date),
     foreign key(t_name) references itp_member(m_name) on DELETE CASCADE
    );

insert into itp_tech values(1,"React","Carrie","리액트", default);
insert into itp_tech values(2, "Spring","Dennis","스프링", default);
insert into itp_tech values(3,"Vue","Maple","뷰", default);
insert into itp_tech values(4,"Vue","Maple","뷰2", default);
insert into itp_tech values(5, "React","Ryan","리액트2", default);
insert into itp_tech values(6,"Spring","Kale","스프링2", default);